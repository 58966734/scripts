...
:BEGIN
echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
echo "winchannel sfa 配置文件参数一览
echo "常用操作：j:java/c:cls/n:netstat/o:notepad/oo:notepad++/q:exit"
echo "d:jmap/k:kill/p:path/l:look                                          "
echo "MAINTAINER LiuLiguo v20170720                               "
...
if "%TOMCAT_DIR%"=="j" ( wmic process where name='java.exe'  get processid,commandline>javasnapshot.txt & start notepad.exe javasnapshot.txt & goto :BEGIN )
...
if "%TOMCAT_DIR%"=="l" ( set /p pid= "请输入要look的进程PID:" & echo %jmap% -heap !pid! & %jmap% -heap !pid! & goto :BEGIN )
...
for /f "tokens=1 delims= " %%i in (p.tmp) do set DIR_part=%%i
for /f "tokens=* delims= " %%i in (p.tmp) do set DIR_short=%%~fsi
if not "%DIR_part%"=="%TOMCAT_DIR%" set TOMCAT_DIR=%DIR_short%
del /f /q p.tmp
